...
<div class="sm:col-span-3 w-full">
    ...
    <div class="py-12 w-full grow rounded-lg drop-shadow-lg">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 h-scren">
            ...
            <a href="{{ route('manuscript.index') }}" class="flex flex-col items-center mb-5 w-full bg-white border border-gray-200 rounded-lg shadow md:flex-row hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
    <img class="object-cover w-full rounded-t-lg md:w-48 md:rounded-none md:rounded-s-lg" src="{{ asset('sys_logo/manuscript.jpg') }}" alt="">
    <div class="flex flex-col justify-between p-4 leading-normal">
        <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
            Submitted Manuscript: {{ $manuscript->title ?? 'Unavailable' }}
            <span class="uppercase 
                @if($manuscript && $manuscript->status === 'approved')
                    text-green-500
                @elseif($manuscript && $manuscript->status === 'pending')
                    text-yellow-500
                @elseif($manuscript && $manuscript->status === 'declined')
                    text-red-500
                @else
                    text-gray-500
                @endif
        ...
        <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">
            Tracking Code: {{ $manuscript->tracking_code ?? 'Unavailable' }}
        </p>
            </div>
        </a>
                 <div class="p-6 h-full bg-white drop-shadow-lg space-y-3 mb-2 rounded-lg">
                 <div class="flex items-center justify-center"> 
                 <h3 class="text-[#1b651b] uppercase text-2xl font-bold">
