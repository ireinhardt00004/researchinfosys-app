...
use App\Models\InventionUtility;
...
class UtilityController extends Controller
{
        ...
        try {
            foreach ($data as $row) {
                ...
                if (empty($missingFields)) {
                    ...
                    InventionUtility::updateOrCreate(
                        ['id' => $row['id'] ?? null],
                        [
                            ...
                            'title_invention_utility_model' => $row['title_invention_utility_model'] ?? '',
                             'patent_number' => $row[ 'patent_number'] ?? '',
                            'date_of_issue' => $row['date_of_issue'] ?? null,
                            'utilization_invention_development' => $row['utilization_invention_development'] ?? '',
                            'utilization_invention_service' => $row['utilization_invention_service'] ?? '',
                            'name_commercial_product' => $row['name_commercial_product'] ?? '',
                            'points' => $row['points'] ?? '',
                            'user_id' => $userID,
                        ]
                    );
            ...
            return $this->downloadInventionUtilityExcelFile($data);
        } catch (\Exception $e) {
    ...
    private function downloadInventionUtilityExcelFile($data)
    {
        try {
            $user = auth()->user();
            $lname = $user->lname;
            ...
            if (!is_dir($folderPath)) {
                ...
                $templateFilePath = public_path('template/invention/MFO-and-KRA-DATA-FORM.xlsx');
            ...
            // Get the sheet named "Inventions or Utility Models"
            $sheet = $spreadsheet->getSheetByName('Inventions or Utility Models');
            if (!$sheet) {
                Log::error("Sheet 'Inventions or Utility Models' not found");
            ...
            $excelFileName = 'Inventions_or_Utility_Models_Export_' . $timestamp . '.xlsx';
            ...
            Dropbox::create([
                'title' => 'Inventions or Utility Models (' . strtoupper(Carbon::now()->format('M')) . ')',
                'user_id' => $userID,
            ...
            ActivityLog::create([
                'user_id' => $userID,
                'activity' => 'sent a report named Inventions or Utility Models (' . strtoupper(Carbon::now()->format('M')) . ')',
            ]);
            ...
            if ($admin) {
                Notification::create([
                    ...
                    'message' => 'sent a report named Inventions or Utility Models (' . strtoupper(Carbon::now()->format('M')) . ')',
                    'is_read' => false,
                ]);
            }
