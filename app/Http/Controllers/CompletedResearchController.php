...
class CompletedResearchController extends Controller
{
    public function store(Request $request)
    {
        ...
        // Define the expected fields for CompletedResearch
        $expectedFields = [
        ...
        try {
            foreach ($data as $row) {
                ...
                if (array_intersect($expectedFields, array_keys($row)) === $expectedFields) {
                    // Create or update the CompletedResearch record
                    CompletedResearch::updateOrCreate(
                        ['id' => $row['id'] ?? null],
                        [
                            ...
                            'funding_cvsu' => $row['funding_cvsu'] ?? '',
                            'funding_external' => $row['funding_external'] ?? '',
                            'ifnt_cvsu_research_type' => $row['ifnt_cvsu_research_type'] ?? '',
                            'cooperating_agency' => $row['cooperating_agency'] ?? '',
            ...
            return $this->downloadCompletedResearchExcelFile($data);
        } catch (\Exception $e) {
            Log::error('Error saving completed research: ' . $e->getMessage());
            return redirect()->back()->withErrors('An error occurred while saving the completed research.');
        }
    }

    private function downloadCompletedResearchExcelFile($data)
    {
        try {
            $user = auth()->user();
            $lname = $user->lname;
            ...
            $folderPath = public_path('sub_files/' . $folderName);
            ...
            if (!is_dir($folderPath)) {
                ...
                $templateFilePath = public_path('template/complete/MFO-and-KRA-DATA-FORM.xlsx');
            ...
            // Get the sheet named "Completed research"
            $sheet = $spreadsheet->getSheetByName('Completed research');
            if (!$sheet) {
                Log::error("Sheet 'Completed research' not found");
            ...
            $excelFileName = 'Completed_Research_Export_' . $timestamp . '.xlsx';
            ...
            Dropbox::create([
                'title' => 'Completed Research (' . strtoupper(Carbon::now()->format('M')) . ')',
                'user_id' => $userID,
               'file_path' => 'export_forms/' . $folderName . '/' . $excelFileName, 
                'status' => 'pending',
            ]);
            ActivityLog::create([
                'user_id' => $userID,
                'activity' => 'sent a report named Completed Research (' . strtoupper(Carbon::now()->format('M')) . ')',
            ]);
            $admin = 'admin';
            ...
            if ($admin) {
                Notification::create([
                    ...
                    'message' => 'sent a report named Completed Research (' . strtoupper(Carbon::now()->format('M')) . ')',
                    'is_read' => false,
                ]);
            }
